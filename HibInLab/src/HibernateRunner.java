...
public class HibernateRunner {
    ...
    private void addNewUsers() {
        ...
        User kobeUser = new User();
        kobeUser.setUname("KobeB");
        kobeUser.setPword("KingCobra");

        User michaelJordan = new User();
        michaelJordan.setUname("MichaelJ");
        michaelJordan.setPword("Best Ever");
        ...
        System.out.println("anotherUser generated ID is: " + kobeUser.getId());
        System.out.println("anotherUser generated ID is: " + michaelJordan.getId());
    }
    ...
    private void modifyUser() {
         ...
         */
        Query singleUserQuery = session.createQuery("select u from User as u where u.uname='MichaelJ'");
        User michaelJordan = (User)singleUserQuery.uniqueResult();
        /*
         * change the user name for the Java instance
         */
        michaelJordan.setUname("Mutombo");
        /*
    ...
    private void addSharedPhoneNumber() {
         ...
         */
        Query joshuaQuery = session.createQuery("select u from User as u where u.uname='Mutombo'");
        User MutomboUser = (User)joshuaQuery.uniqueResult();

        Query aNameQuery = session.createQuery("select u from User as u where u.uname='KobeB'");
        User kobeUser = (User)aNameQuery.uniqueResult();
        ...
        */
        ...
        Set<PhoneNumber> MutomboUserPhoneNumbers= MutomboUser.getPhoneNumbers();
        MutomboUserPhoneNumbers.add(sharedPhoneNumber);
       /*
        * set the single phone number to be used by more than one User
        */
        Set<PhoneNumber> KobePhoneNumbers = kobeUser.getPhoneNumbers();
        KobePhoneNumbers.add(sharedPhoneNumber);
       /*
        ...
        */
        session.merge(MutomboUser);
        session.merge(kobeUser);
       /*
        * permanently store the changes into the database tables.
        */
        transaction.commit();
